...
/// Returns (supported_kernels, unsupported_count, boot_partition) where
/// supported_kernels is an ordered list of kernel paths.
fn inspect_source_image_boot_data(
    g: &guestfs::Handle,
    source_rootfs: &str,
) -> Result<(Vec<String>, usize, BootPartition)> {
    ...
    Ok((supported, unsupported_count, boot_partition))
}

/// Extracts kernel, initrd, and kernel parameters from the target
/// image. Reads grub.cfg fresh from the target (after update-grub
/// ran) and classifies entries as supported or unsupported using the
/// kernel paths from the source SEV check.
...
fn extract_boot_data(
    ...
    // Read and parse the target grub.cfg, which was regenerated by update-grub
    // inside install_snpguard_on_target() and reflects the final configuration
    // (UUID/PARTUUID disabled, cryptomount added, etc.).
    let target_grub_content = g
        .read_file("/boot/grub/grub.cfg")
        .map_err(|e| anyhow!("Failed to read target grub.cfg: {:?}", e))?;
    let target_grub_str = String::from_utf8(target_grub_content)
        .map_err(|e| anyhow!("Failed to decode target grub.cfg: {}", e))?;
    let target_entries = grub::parse_grub_cfg_from_str(&target_grub_str, true)
        .map_err(|e| anyhow!("Failed to parse target grub.cfg: {}", e))?;

    // Classify target entries using the set of kernel paths that passed the
    // SEV-SNP check on the source image.
    let supported_set: std::collections::HashSet<&str> =
        supported_kernels.iter().map(|s| s.as_str()).collect();
    let mut target_supported: Vec<grub::GrubEntry> = Vec::new();
    let mut target_unsupported: Vec<grub::GrubEntry> = Vec::new();
    for entry in target_entries {
        if supported_set.contains(entry.kernel.as_str()) {
            target_supported.push(entry);
        } else {
            target_unsupported.push(entry);
        }
    }
    ...
    if !target_supported.is_empty() {
        println!("  SEV-SNP supported entries (selectable):");
        for (idx, entry) in target_supported.iter().enumerate() {
    ...
    if !target_unsupported.is_empty() {
        println!("\n  SEV-SNP unsupported entries (excluded from selection):");
        for entry in &target_unsupported {
            println!("    Kernel: {} (SEV-SNP not supported)", entry.kernel);
        }
    }

    // Select entry from supported list only
    let selected_entry: grub::GrubEntry = if target_supported.is_empty() {
        bail!("No SEV-SNP supported kernels found in target grub.cfg. Cannot proceed.");
    } else if target_supported.len() == 1 {
        let entry = target_supported.remove(0);
        ...
        // Auto-select GRUB default, fallback to first
        let selected_idx = target_supported
            .iter()
            .position(|e| e.is_default)
            .unwrap_or(0);
        let reason = if target_supported[selected_idx].is_default {
            "GRUB default"
        } else {
            "first available"
        };
        println!("\n  Auto-selected entry {} ({}):", selected_idx, reason);
        let entry = target_supported.remove(selected_idx);
        println!("    Kernel: {}", entry.kernel);
        if let Some(ref initrd) = entry.initrd {
            println!("    Initrd: {}", initrd);
        } else {
        ...
        // Ask user to select (--interactive mode)
        print!(
            "\n  Enter entry number (0-{}): ",
            target_supported.len() - 1
        );
        io::stdout()
            .flush()
        ...
        if selected_idx >= target_supported.len() {
            bail!(
    ...
    Ok((kernel_data, initrd_data, selected_entry.params))
}
...
fn run_convert(
    ...
    let (supported_kernels, unsupported_count, boot_partition) =
        inspect_source_image_boot_data(&g, &source_rootfs)?;

    // Check if any kernels support SEV-SNP
    if supported_kernels.is_empty() {
        bail!(
